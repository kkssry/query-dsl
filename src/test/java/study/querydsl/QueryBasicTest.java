...
//
//        // 처음 한 건 조회
//        Member findMember2 = queryFactory.selectFrom(QMember.member)
//                                         .fetchFirst();
    ...
    /*
        세타 조인 (연관 관계가 없는 필드로 조인) -> cross join (N x M 조인)
    ...
    /*
        ...
        SQL : select m.*, t.* from member m left join team t on m.team.id = t.id and t.name = 'teamA';
     */
    @Test
    ...
    /*
        fetch join : 즉시로딩으로 Member, Team SQL 쿼리 조인으로 한번에 조회
     */
    ...
    void subQueryAvg() {
        ...
        List<Tuple> fetch = queryFactory.select(member.username, select(memberSub.age.avg()).from(memberSub))
                                        .from(member)
                                        .fetch();
    ...
    @Test
    void constant_value2() {
        String result = queryFactory.select(member.username.concat("_")
